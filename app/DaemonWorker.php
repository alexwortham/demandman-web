...
class DaemonWorker
{
        ...
        while (true) {
            if ($this->daemonShouldRun()) {
                $this->runNextJobForDaemon(
                    $connectionName, $queue, $delay, $sleep, $maxTries, $memory
                );
            } else {
                $this->sleep($sleep);
            }
     ...
     * @return void
     */
    protected function runNextJobForDaemon($connectionName, $queue, $delay, $sleep, $maxTries, $memory)
    {
        try {
            $this->pop($connectionName, $queue, $delay, $sleep, $maxTries, $memory);
        } catch (Exception $e) {
	    echo $e->getMessage();
     ...
     * @return array
     */
    public function pop($connectionName, $queue = null, $delay = 0, $sleep = 3, $maxTries = 0, $memory)
    {
        ...
        if (! is_null($job)) {
		$job->release(0);
        	$proc = $this->listener->makeProcess($connectionName, $queue, $delay, $memory, 60);
		$this->listener->runProcess($proc, $memory);
        }
